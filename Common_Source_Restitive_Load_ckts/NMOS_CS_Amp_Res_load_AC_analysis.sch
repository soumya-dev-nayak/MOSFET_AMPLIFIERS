...
device=resistor
m=1}
C {vsource.sym} -200 -10 0 0 {name=V1 value="dc 0.835 ac 1 sin(0 1m 1k)" savecurrent=false}
...
*-3 db level / Pole frequency
let A3db = A0-3

*Finding the 3 dB bandwidth
meas ac BW3dB when gain_db = A3dB

* Calculate d(Gain)/d(log10(frequency))
let slope = deriv(gain_db)/deriv(log10(frequency))

* Zero frequency:
* At the zero, slope changes from about -20 dB/dec
* toward 0 dB/dec, so it crosses -10 dB/dec.
meas ac fz when slope=-10 cross=2
